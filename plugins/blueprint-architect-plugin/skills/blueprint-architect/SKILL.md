...
10. Save the draft specification as JSON, then run `node --experimental-strip-types "<skill-directory>/scripts/compatibility-graph.ts" "<draft-blueprint-spec.json>" "<skill-directory>/references/capability-rules.yaml" "<analyzed-blueprint-spec.json>"`. This command validates the specification, evaluates the complete graph, and writes the analyzed specification. Present its final findings, evidence confidence, unresolved items, and proposed directory tree. Request final confirmation under the same selector-or-numbered-fallback contract.
11. Only after confirmation, run `node --experimental-strip-types "<skill-directory>/scripts/generate-structure.ts" "<analyzed-blueprint-spec.json>" "<output-directory>"`. The generator creates the declared tree and complete module/interface documentation atomically and never overwrites an existing target.
